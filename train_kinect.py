...
from internal import configs, datasets_depth_kinect, math, models, utils, vis  # pylint: disable=g-multiple-import
import jax
...
#   print('step:', step)
  def loss_fn(variables): # contains global vars of train_step, such as batch...
    ...
    # sparse_depth_losses = jnp.array(sparse_depth_losses)
  ...
  # sparse_depth_losses = jax.lax.pmean(sparse_depth_losses, axis_name='batch')
  ...
  stats = TrainStats(
      ...
      # sparse_depth_losses=sparse_depth_losses,
      disp_mses=disp_mses,
...
def main(unused_argv):
  ...
  dataset = datasets_depth_kinect.load_dataset('train', config.data_dir, config)
  test_dataset = datasets_depth_kinect.load_dataset('test', config.data_dir, config)
    ...
    if jax.host_id() == 0:
      ...
      if step % config.print_every == 0:
        ...
        print(f'{step:{precision}d}' + f'/{config.max_steps:d}: ' +
              f'loss={stats.loss:0.4f}, ' + f'avg_psnr={avg_psnr:0.2f}, ' + #f'sparse_depth_losses={stats.sparse_depth_losses[-1]:0.4f}, ' + #f'sparse_depth_losses={stats.sparse_depth_losses:0.4f}, ' +

...
class MipNerfModel(nn.Module):
  ...
  def __call__(
    ...
    for i_level in range(self.num_levels):
      ...
      # Volumetric rendering. weights, alpha, trans, delta
      weights, alpha, trans, delta = mip.compute_alpha_weights( 
          density, t_vals, rays.directions)
      rendering, distance_mean = mip.volumetric_rendering(
  ...
  # def predict_scale_bias(predicted_depth): #monodepth
  #   return scaled_depth
...
def construct_mipnerf(rng, rays, config):
  ...
  ray = jax.tree_map(lambda x: jnp.reshape(x, [-1, x.shape[-1]])[:12], rays) 

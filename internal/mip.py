...
def volumetric_rendering(rgbs, weights, normals, t_vals, white_background,
                         vis_num_rays, compute_extras, delta, alpha, trans):
  ...
  if compute_extras:
    ...
    rendering['distance_std'] = jnp.nan_to_num(
        jnp.maximum(
            0.,
            jnp.sqrt(expectation(t_mids**2) - distance_mean**2)),
        0.)
  ...
  return rendering, distance_mean
...
def resample_along_rays(rng,
  ...
  means, covs = cast_rays(new_t_vals, origins, directions, radii, ray_shape, diag=diag)

...
class Config:
  ...
  batch_size: int = 512#1024  # The number of rays/pixels in each batch.
  batch_size_random: int = 512#1024  # The number of rays/pixels in each batch.
  ...
  render_path_frames: int = 180  # Number of frames in path. Used only by LLFF.
  # render_path_frames: int = 10  # Number of frames in path. Used only by LLFF.
  ...
  render_chunk_size: int = 1024 #4096#4096  # Chunk size for whole-image renderings.
  ...
  depth_tvnorm_loss_mult: float = 0.1  # Loss weight of depth tv norm.
